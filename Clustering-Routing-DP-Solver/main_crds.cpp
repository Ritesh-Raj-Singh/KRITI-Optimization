...
public:
    RouteResult calculate_optimal_route(Vehicle v, vector<Passenger> &cluster)
    {
        ...
        for (int i = 1; i < NODES; i++)
        {
            ...
            /* ===== BASE OBJECTIVE ===== */
            double base_score =
                (trip_cost * GLOBAL_CONFIG.obj_cost_weight) + (total_passenger_time * GLOBAL_CONFIG.obj_time_weight);

            /* ===== PENALTIES ===== */
            double penalties =
                (GLOBAL_CONFIG.alpha * ride_time_violation) + (GLOBAL_CONFIG.beta * time_window_violation) + (GLOBAL_CONFIG.gamma * capacity_violation);
