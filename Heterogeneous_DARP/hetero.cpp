...
public:
    ...
    RouteMetrics evaluate(const std::vector<Vehicle>& vehicles, const std::vector<Request>& requests, const Config& config) {
        ...
        for (auto& node : sequence) {
            ...
            // Note: Inline math replaces the old string-based `getTravelTimeFromMatrix`
            // Assumes speed is in km/h and we need minutes: (dist / speed) * 60
...
public:
    ...
    void calculateTotalScore(const std::vector<Vehicle>& vehicles, const std::vector<Request>& requests, const Config& config) {
        ...
        score += unassigned_requests.size() * 10000000.0;
        total_score = score;
    ...
    void repair(Solution& sol) {
        ...
        for (int emp_idx : unassigned) {
            ...
            for (size_t r = 0; r < sol.routes.size(); ++r) {
                ...
                if (m.objective_score < base_score + 100000000.0) { 
...
int main(int argc, char **argv) {
    if (argc < 5) {
        std::cerr << "Usage: " << argv[0] << " <vehicles.csv> <requests.csv> <metadata.csv> <matrix.txt>\n";
        return 1;
    }
    ...
    Config config;
    std::cout << "Loading metadata from " << argv[3] << "...\n";
    loadMetadata(argv[3], config);

    std::cout << "Loading data...\n";
    std::vector<Vehicle> vehicles = loadVehicles(argv[1]);
    std::vector<Request> requests = loadRequests(argv[2], config.max_delays);
    ...
    std::cout << "Loading matrix from " << argv[4] << " (Expecting " << matrix_size << "x" << matrix_size << ")...\n";
    ...
    std::ofstream emp_file("output_employees.csv");
    std::ofstream veh_file("output_vehicle.csv");
